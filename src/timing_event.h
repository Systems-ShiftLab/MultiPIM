...
class TimingEvent {
    ...
    public:
        ...
        virtual void parentDonePTW(uint64_t startCycle, uint64_t firstPTWStartCycle); // see cpp
        ...
        void donePTW(uint64_t doneCycle, uint64_t firstPTWStartCycle) {
            assert(state == EV_RUNNING); //ContentionSim sets it when calling simulate()
            state = EV_DONE;
            auto vLambda = [this, doneCycle, firstPTWStartCycle](TimingEvent** childPtr) {
                checkDomain(*childPtr);
                (*childPtr)->parentDonePTW(doneCycle+postDelay,firstPTWStartCycle);
            };
