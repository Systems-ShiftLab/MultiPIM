...
void Cache::addChild(BaseCache* child, Network* network) {
     cc->addChild(child, network);
}

void Cache::addChildren(const g_vector<BaseCache*>& children, Network* network){
...
uint64_t Cache::access(MemReq& req) {
    ...
    // printf("before cache:access 0x%x\n",req.lineAddr);
    ...
    if (likely(!skipAccess)) {
        if(zinfo->cacheWritePolicy == WritePolicy::WRITETHROUGH){
            ...
            uint64_t tempStart = respCycle;
            respCycle = cc->processAccess(req, lineId, respCycle);
            if(respCycle == tempStart){
                //hit
                req.l1hit = true;
            }else {
                //miss
            ...
            int32_t lineId = array->lookup(req.lineAddr, &req, updateReplacement);
            respCycle += accLat;
            // printf("cache:access 0x%x lineId %d\n",req.lineAddr, lineId);
...
uint64_t Cache::clflush_all( const InvReq& req)
{
	uint64_t respCycle = req.cycle;
	MemObject* obj= cc->getParent(req.lineAddr);
    MemObject *pre = obj;

    // get LLC, invalidate from LLC to L1
    while (dynamic_cast<BaseCache *>(obj))
    {
        pre = obj;
        obj = dynamic_cast<Cache *>(obj)->cc->getParent(req.lineAddr);
    }
    if (pre)
    {
        // dynamic_cast<Cache*>(pre)->cc->endInv(); //in case deadlock
        // dynamic_cast<Cache*>(pre)->cc->startInv(); //in case deadlock

        respCycle = dynamic_cast<Cache *>(pre)->invalidate(req);

        // dynamic_cast<Cache*>(pre)->cc->endInv();
