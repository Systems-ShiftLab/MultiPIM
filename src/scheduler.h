...
class Scheduler : public GlobAlloc, public Callee {
    private:
        ...
        enum PIMThreadScheduler {
            RANDOM = 0,
            GROUP = 1,
            MAX
        };
        PIMThreadScheduler pimScheduler;
        ...
        g_vector<g_unordered_set<uint32_t>> pinCoreStatus;
        g_unordered_map<uint32_t, uint32_t> threadPinCoreMap;//<thread_id,cid>
    ...
    public:
        Scheduler(void (*_atSyncFunc)(void), uint32_t _parallelThreads, uint32_t _numCores, uint32_t _schedQuantum, g_string& pimSchedulerName) :
            atSyncFunc(_atSyncFunc), bar(_parallelThreads, this), numCores(_numCores), schedQuantum(_schedQuantum), rnd(0x5C73D9134)
        {
            pinCoreStatus.resize(numCores);
            if(pimSchedulerName == "Random"){
                pimScheduler = RANDOM;
            }else if(pimSchedulerName == "Group"){
                pimScheduler = GROUP;
            }else{
                info("Unsupported scheduler!")
                assert(0);
            }
        ...
        BBLStatus& getBBLStatus(uint32_t pid, uint32_t tid){
            uint32_t gid = getGid(pid, tid);
            assert((gidMap.find(gid) != gidMap.end()));
            return gidMap[gid]->bblStatus;
        }

        void saveRegScoreBoard(uint32_t pid, uint32_t tid, uint64_t (&_regScoreboard)[MAX_REGISTERS]){
            uint32_t gid = getGid(pid, tid);
            if(gidMap.find(gid) != gidMap.end()){
                uint64_t (&regScoreboard)[MAX_REGISTERS] = gidMap[gid]->regScoreboard;
                for (uint32_t i = 0; i < MAX_REGISTERS; i++){
                    regScoreboard[i] = _regScoreboard[i];
                }
            }
        }
        void restoreRegScoreBoard(uint32_t gid, uint64_t (&_regScoreboard)[MAX_REGISTERS]){
            assert((gidMap.find(gid) != gidMap.end()));
            uint64_t (&regScoreboard)[MAX_REGISTERS] = gidMap[gid]->regScoreboard;
            for (uint32_t i = 0; i < MAX_REGISTERS; i++){
                _regScoreboard[i] = regScoreboard[i];
            }
        }

        void enterOffloadRegion(uint32_t pid, uint32_t tid){
            uint32_t gid = getGid(pid, tid);
            assert((gidMap.find(gid) != gidMap.end()));
            gidMap[gid]->inOffloadRegion = true;
        }
        void exitOffloadRegion(uint32_t pid, uint32_t tid){
            uint32_t gid = getGid(pid, tid);
            assert((gidMap.find(gid) != gidMap.end()));
            gidMap[gid]->inOffloadRegion = false;
        }
        bool isInOffloadRegion(uint32_t pid, uint32_t tid){
            uint32_t gid = getGid(pid, tid);
            if(gidMap.find(gid) == gidMap.end()){
                return false;
            }else{
                assert(gidMap[gid]!=NULL);
                return gidMap[gid]->inOffloadRegion;
            }
        }
        ...
        void randomScheduler(uint32_t gid,const g_vector<bool>& mask, g_vector<bool>& new_mask){
            std::vector<uint32_t> availVec;
            availVec.resize(numCores);
            for (uint32_t i = 0; i < numCores; i++) availVec[i] = i;

            //Random shuffle (Fisher-Yates)
            for (uint32_t i = numCores - 1; i > 0; i--) {
            ...
            for (std::list<uint32_t>::iterator it = avail.begin(); it != avail.end(); it++) {
                uint32_t cid = *it;
                if(mask[cid] && pinCoreStatus[cid].size() == 0){
                    new_mask[cid] = true;
        ...
        void groupScheduler(uint32_t gid,const g_vector<bool>& mask, g_vector<bool>& new_mask){
            std::vector<uint32_t> availVec;
            availVec.resize(numCores);
            for (uint32_t i = 0; i < numCores; i++) availVec[i] = i;
        ...
        void schedulePIMThread(uint32_t gid, const g_vector<bool>& mask, g_vector<bool>& new_mask){
            assert(new_mask.size() == numCores);

            if(threadPinCoreMap.find(gid) != threadPinCoreMap.end()){
                uint32_t cid = threadPinCoreMap[gid];
                assert(cid >= 0 && cid < numCores);
                assert(pinCoreStatus[cid].find(gid) != pinCoreStatus[cid].end());
                new_mask[cid] = true;
                return;
            }
            ...
            switch(pimScheduler){
                case RANDOM:
                    return randomScheduler(gid,mask,new_mask);
                case GROUP:
                    return groupScheduler(gid,mask,new_mask);;
                default:
                    info("Unsupported scheduler!")
                    assert(0);
                    break;
        ...
        void start(uint32_t pid, uint32_t tid, const g_vector<bool>& mask) {
            ...
            if(zinfo->enable_pim_mode){
                ...
                gidMap[gid] = new ThreadInfo(gid, syscall(SYS_getpid), syscall(SYS_gettid), new_mask);
            }else{
        ...
        void finish(uint32_t pid, uint32_t tid) {
            ...
            if(threadPinCoreMap.find(gid) != threadPinCoreMap.end()){
                uint32_t map_cid = threadPinCoreMap[gid];
                pinCoreStatus[map_cid].erase(gid);
                threadPinCoreMap.erase(gid);
            }
