...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId, uint32_t coreId, Address readAddr, uint32_t threadId, bool isPIMInst) {
    ...
    switch (*state) {
        ...
        case E:
            {
                MemReq req = {wbLineAddr, PUTS, selfId, state, cycle, &ccLock, *state, srcId, 0 /*no flags*/, coreId,readAddr};
        ...
        case M:
            {
                MemReq req = {wbLineAddr, PUTX, selfId, state, cycle, &ccLock, *state, srcId, 0 /*no flags*/, coreId, readAddr};
                req.threadId = threadId;
                req.isPIMInst = isPIMInst;
                respCycle = parents[getParentId(wbLineAddr)]->access(req);
            }
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, uint32_t coreId, uint32_t threadId, bool isPIMInst) {
    uint64_t respCycle = cycle;
    MESIState* state = &array[lineId];
    switch (type) {
        ...
        case GETS:
            ...
            if (*state == I) {
                ...
                profGETSMiss.inc();
                // printf("Line 0x%x in child %d is set\n",lineAddr,selfId);
                assert(*state == S || *state == E);
            } else {
                // printf("Line 0x%x in child %d hit\n",lineAddr,selfId);
                profGETSHit.inc();
            }
            break;
        case GETX:
            // printf("Child %d getx line 0x%x\n",selfId,lineAddr);
            if (*state == I || *state == S) {
...
void MESIBottomCC::processInval(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, bool cl_flush) {
    MESIState* state = &array[lineId];
    if(!cl_flush)
        assert(*state != I);
    else if(*state == I)
...
uint64_t MESIBottomCC::processNonInclusiveWriteback(Address lineAddr, AccessType type, uint64_t cycle, MESIState* state, uint32_t srcId, uint32_t flags, uint32_t coreId, uint32_t threadId, bool isPIMInst) {
    ...
    MemReq req = {lineAddr, type, selfId, state, cycle, &ccLock, *state, srcId, flags | MemReq::NONINCLWB, coreId};
    req.threadId = threadId;
    req.isPIMInst = isPIMInst;
...
void MESITopCC::add(BaseCache* child, Network* network, const char* name) {
...
void MESITopCC::add(const g_vector<BaseCache*>& _children, Network* network, const char* name) {
    uint32_t size = children.size();
...
uint64_t MESITopCC::sendInvalidates(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, uint64_t cycle, uint32_t srcId, bool is_clflush) {
...
uint64_t MESITopCC::processEviction(Address wbLineAddr, uint32_t lineId, bool* reqWriteback, uint64_t cycle, uint32_t srcId, uint32_t coreId, Address readAddr) {
    if (nonInclusiveHack) {
...
uint64_t MESITopCC::processInval(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, uint64_t cycle, uint32_t srcId, bool is_clflush) {
    if (type == FWD) {//if it's a FWD, we should be inclusive for now, so we must have the line, just invLat works
        ...
        return sendInvalidates(lineAddr, lineId, type, reqWriteback, cycle, srcId, is_clflush);
    }
}

uint32_t IVBottomCC::getParentId(Address lineAddr) {
    //Hash things a bit
    uint32_t res = 0;
    uint64_t tmp = lineAddr;
    for (uint32_t i = 0; i < 4; i++) {
...
uint32_t IVBottomCC::getParentId(uint32_t srcId,Address lineAddr) {
    return (srcId % parents.size());
}

void IVBottomCC::init(const g_vector<MemObject*>& _parents, Network* network, const char* name) {
    parents.resize(_parents.size());
    parentRTTs.resize(_parents.size());
    for (uint32_t p = 0; p < parents.size(); p++) {
        parents[p] = _parents[p];
...
uint64_t IVBottomCC::processAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, uint32_t coreId, uint32_t threadId, bool isPIMInst) {
    uint64_t respCycle = cycle;
    MESIState* state = &array[lineId];
    switch (type) {
        // A PUTS/PUTX does nothing w.r.t. higher coherence levels --- it dies here
        ...
        case GETS:
            if (*state == I) {
                ...
                uint32_t nextLevelLat = parents[parentId]->access(req) - cycle;
                uint32_t netLat = parentRTTs[parentId];
                profNextLevelLat.inc(nextLevelLat);
                profNetLat.inc(netLat);
                respCycle += nextLevelLat + netLat;
                profGETSMiss.inc();

                *state = S; //midify state directly
            } else {
                profGETSHit.inc();
            }
            break;
        case GETX:
            // Always write to memory
            {
                ...
                //Profile before access, state changes
                uint32_t parentId = getParentId(srcId,lineAddr);
                MemReq req = {lineAddr, PUTX, selfId, state, cycle, &ccLock, *state, srcId, flags, coreId};
                req.threadId = threadId;
                req.isPIMInst = isPIMInst;
                uint32_t nextLevelLat = parents[parentId]->access(req) - cycle;
                uint32_t netLat = parentRTTs[parentId];
...
void IVBottomCC::processInval(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, bool cl_flush){
    MESIState* state = &array[lineId];
    if(*state == I)
        return;
    switch (type) {
