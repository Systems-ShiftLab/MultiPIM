...
public:
    ...
    virtual long page_allocator(long addr, int coreid){};
    virtual void record_core(int coreid) = 0;
    virtual int get_route_link(int cur_cub, int target_cub) = 0;
    virtual long get_total_mem_req() = 0;
    virtual long get_mem_size() = 0;
    virtual int get_hmc_stacks(){return 0;}
    virtual void set_group_idx(int idx){}
    virtual int get_target_vault(long addr){assert(0); return -1;}
    virtual int get_target_stack(long addr){assert(0); return -1;}
    virtual int memAccsssPosition(int coreid, long req_addr, bool ideal_memnet){ assert(0); return -1; }
    virtual int estimateMemHops(int coreid, long addr, bool is_pim, bool ideal_memnet){assert(0); return -1;}
};
...
public:
    ...
    int get_route_link(int cur_cub, int target_cub){return -1;}
