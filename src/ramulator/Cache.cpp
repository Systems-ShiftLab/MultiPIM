...
Cache::Cache(int size, int assoc, int block_size,
    ...
    std::shared_ptr<CacheSystem> cachesys, std::string _src_id_str, bool pim_mode):
  ...
  // regStats
  cache_read_hit.name(level_string + string("_cache_read_hit") + src_id_str)
                 .desc("cache read hit count")
                 .precision(0)
                 ;
  cache_write_hit.name(level_string + string("_cache_write_hit") + src_id_str)
                 .desc("cache write hit count")
                 .precision(0)
                 ;
  cache_total_hit.name(level_string + string("_cache_total_hit") + src_id_str)
                 .desc("cache total hit count")
                 .precision(0)
                 ;
  cache_read_miss.name(level_string + string("_cache_read_miss") + src_id_str)
                 .desc("cache read miss count")
                 .precision(0)
                 ;

  cache_write_miss.name(level_string + string("_cache_write_miss") + src_id_str)
                  .desc("cache write miss count")
                  .precision(0)
                  ;

  cache_total_miss.name(level_string + string("_cache_total_miss") + src_id_str)
                  .desc("cache total miss count")
                  .precision(0)
                  ;

  cache_eviction.name(level_string + string("_cache_eviction") + src_id_str)
                .desc("number of evict from this level to lower level")
                .precision(0)
                ;

  cache_read_access.name(level_string + string("_cache_read_access") + src_id_str)
                  .desc("cache read access count")
                  .precision(0)
                  ;

  cache_write_access.name(level_string + string("_cache_write_access") + src_id_str)
                    .desc("cache write access count")
                    .precision(0)
                    ;

  cache_total_access.name(level_string + string("_cache_total_access") + src_id_str)
                    .desc("cache total access count")
                    .precision(0)
                    ;

  cache_mshr_hit.name(level_string + string("_cache_mshr_hit") + src_id_str)
                .desc("cache mshr hit count")
                .precision(0)
                ;
  cache_mshr_unavailable.name(level_string + string("_cache_mshr_unavailable") + src_id_str)
                         .desc("cache mshr not available count")
                         .precision(0)
                         ;
  cache_set_unavailable.name(level_string + string("_cache_set_unavailable") + src_id_str)
                         .desc("cache set not available")
                         .precision(0)
                         ;

  cache_read_data_hit.name(level_string + string("_cache_read_data_hit") + src_id_str)
                 .desc("cache read data hit count")
                 .precision(0)
                 ;
  cache_write_data_hit.name(level_string + string("_cache_write_data_hit") + src_id_str)
                 .desc("cache write data hit count")
                 .precision(0)
                 ;
  cache_total_data_hit.name(level_string + string("_cache_total_data_hit") + src_id_str)
                 .desc("cache total data hit count")
                 .precision(0)
                 ;
  cache_read_data_miss.name(level_string + string("_cache_read_data_miss") + src_id_str)
                 .desc("cache read data miss count")
                 .precision(0)
                 ;

  cache_write_data_miss.name(level_string + string("_cache_write_data_miss") + src_id_str)
                  .desc("cache write data miss count")
                  .precision(0)
                  ;

  cache_total_data_miss.name(level_string + string("_cache_total_data_miss") + src_id_str)
                  .desc("cache total data miss count")
                  .precision(0)
                  ;

  cache_read_data_access.name(level_string + string("_cache_read_data_access") + src_id_str)
                  .desc("cache read data access count")
                  .precision(0)
                  ;

  cache_write_data_access.name(level_string + string("_cache_write_data_access") + src_id_str)
                    .desc("cache write data access count")
                    .precision(0)
                    ;

  cache_total_data_access.name(level_string + string("_cache_total_data_access") + src_id_str)
                    .desc("cache total data access count")
                    .precision(0)
                    ;
}
...
bool Cache::send(Request req) {
  ...
  if(!req.instruction_request)
    cache_total_data_access++;
  if (req.type == Request::Type::WRITE) {
    cache_write_access++;
    cache_write_data_access++;
  } else {
    assert(req.type == Request::Type::READ);
    cache_read_access++;
    if(!req.instruction_request)
      cache_read_data_access++;
  }
  ...
  if (is_hit(lines, req.addr, &line)) {
    ...
    cache_total_hit++;
    if(!req.instruction_request)
      cache_total_data_hit++;
    if (req.type == Request::Type::WRITE) {
      cache_write_hit++;
      cache_write_data_hit++;
    } else {
      cache_read_hit++;
      if(!req.instruction_request)
        cache_read_data_hit++;
    ...
    cache_total_miss++;
    if(!req.instruction_request)
      cache_total_data_miss++;
    if (req.type == Request::Type::WRITE) {
      cache_write_miss++;
      if(!req.instruction_request)
        cache_write_data_miss++;
    } else {
      assert(req.type == Request::Type::READ);
      cache_read_miss++;
      if(!req.instruction_request)
        cache_read_data_miss++;
    }
    ...
    if (mshr != mshr_entries.end()) {
      ...
      // return true; // should not return true
      return false; // Return false， assuse each mshr entry can only host onlye one request
    }
...
std::pair<long, bool> Cache::invalidate(long addr, bool clflush) {
...
void Cache::evict(std::list<Line>* lines,
  ...
  else
  {
    ...
    if (dirty)
    {
      if (is_private_cache)
      {
        Request write_req(addr, Request::Type::WRITE, src_id, true);
...
void Cache::printHitInfo(){
	int readhits = cache_read_access.value() - cache_read_miss.value();
	int writehits = cache_write_access.value() - cache_write_miss.value();
	printf("Hit: %d, Miss: %d\n", readhits + writehits, cache_read_miss.value() + cache_write_miss.value());
}
