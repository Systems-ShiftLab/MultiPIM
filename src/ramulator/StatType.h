...
class Distribution: public Stat<Distribution> {
 ...
 public:
  ...
  void print(std::ofstream& file) {
    Stat<Distribution>::printname(file);
    file.precision(Stat<Distribution>::_precision);
    file.width(20);
    file << std::fixed << sum <<","<<samples<<","<<((double)sum)/samples<<","<< min_track<<","<<max_track<<","<<bucket_size;
    Stat<Distribution>::printdesc(file);
...
class Histogram: public Stat<Histogram> {
 ...
 public:
  ...
  void print(std::ofstream& file) {
    Stat<Histogram>::printname(file);
    file.precision(Stat<Histogram>::_precision);
    file.width(20);
    file << std::fixed << sum <<","<<samples<<","<<((double)sum)/samples<<","<< min_bucket<<","<<max_bucket<<","<<bucket_size;
    Stat<Histogram>::printdesc(file);
