...
class Cache: public GlobAlloc
{
public:
  ScalarStat cache_read_hit;
  ScalarStat cache_write_hit;
  ScalarStat cache_total_hit;
  ...
  ScalarStat cache_read_data_hit;
  ScalarStat cache_write_data_hit;
  ScalarStat cache_total_data_hit;
  ScalarStat cache_read_data_miss;
  ScalarStat cache_write_data_miss;
  ScalarStat cache_total_data_miss;
  ScalarStat cache_read_data_access;
  ScalarStat cache_write_data_access;
  ScalarStat cache_total_data_access;
public:
  enum class Level {
    L1,
    L2,
  ...
  Cache(int size, int assoc, int block_size, int mshr_entry_num,
      Level level, std::shared_ptr<CacheSystem> cachesys, std::string _src_id_str, bool pim_mode);
  ...
  std::pair<long, bool> invalidate(long addr, bool clflush = false);
...
protected:
  ...
  // It will also evict cachelines of caches in other PUs and CPUs 
...
class CacheSystem : public GlobAlloc
{
public:
  CacheSystem(const Config& configs, std::function<bool(Request)> send_memory):
    send_memory(send_memory) {
      ...
      } else if (configs.has_core_caches() && configs.has_l2_core_cache()) {
