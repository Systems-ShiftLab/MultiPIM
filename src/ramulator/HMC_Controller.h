...
public:
    ...
    //PIM
    function<void(Request&)> send_to_pu;
    deque<Packet> pim_remote_packets_buffer;
    int max_pim_remote_packets_buffer_size = 0;
    // deque<Packet> pim_response_packets_buffer;
    // int max_pim_response_packets_buffer_size = 0;
    bool next_response_packet = true;
        ...
        refresh(new Refresh<HMC>(this))
    {
        ...
        if(pim_mode_enabled){
            max_pim_remote_packets_buffer_size = configs.get_pu_rmab_size();
            // max_pim_response_packets_buffer_size = configs.get_pu_rmab_size();
        }else{
            max_pim_remote_packets_buffer_size = 0;
            // max_pim_response_packets_buffer_size = 0;
        }
        flit_size = channel->spec->flit_bits / 8;
        vaults_per_stack = configs.get_vaults_per_stack();
        ...
        cpu_core_num = configs.get_cpu_core_num();
        cubIdx = channel->id/configs.get_vaults_per_stack();
        deviceIdx = configs.get_int_value("stack_links") + (channel->id % configs.get_vaults_per_stack());
    ...
    bool receive (Packet& packet) {
      ...
      req.transaction_bytes = channel->spec->payload_flits * flit_size;
      lock();
      if (enqueue(req))
      {
          debug_hmc("req.burst_count %d", req.burst_count);
          debug_hmc("req.reqid %d, req.coreid %d, req.pu_id %d", req.reqid, req.coreid, req.pu_id);
    ...
    bool can_receive(Packet& packet){
        Request& req = packet.req;
    ...
    bool receive_remote_request(Packet& packet){
        lock();
        assert(packet.type == Packet::Type::REQUEST);
        if (pim_remote_packets_buffer.size() < max_pim_remote_packets_buffer_size){
            pim_remote_packets_buffer.push_back(packet);
            unlock();
            return true;
        }
        unlock();
    ...
    void set_pu_callback(function<void(Request&)> callback){ send_to_pu = callback;}

    void receive_pim_response (Packet& packet){
        assert(packet.type == Packet::Type::RESPONSE);
        send_to_pu(packet.req);
    }
    ...
    bool enqueue(Request& req)
    {
        ...
        Queue& queue = get_queue(req.type);
        // if(!pim_mode_enabled || (pim_mode_enabled && !req.is_pim_inst)){
        if (queue.max == queue.size())
            return false;
    ...
    inline bool switch_ejection_buffer_empty(){ return icnt_top(cubIdx, deviceIdx) == NULL; }
    inline Packet* switch_eject_packet() { return  (Packet*)icnt_pop(cubIdx, deviceIdx); }
    inline Packet* switch_ejection_buffer_top() { return (Packet*)icnt_top(cubIdx, deviceIdx); }
    inline void switch_ejection_buffer_pop()
    { 
        Packet* packet =  (Packet*)icnt_pop(cubIdx, deviceIdx);
        if(packet != NULL)
            delete packet;
        // printf("Cub %d Vault %d ejected a packet\n",cubIdx, deviceIdx);
    }
    inline bool switch_injection_buffer_full(int size){ return !icnt_has_buffer(cubIdx, deviceIdx,size); }
    inline void switch_inject_packet(Packet& packet, int size, int target_device){
        Packet* data = new Packet();
        *data = packet;
        // printf("Cub %d Vault %d injecting a packet to %d\n",cubIdx, deviceIdx, target_device);
        icnt_push(cubIdx, deviceIdx, target_device, (void*)data, size, packet.type == Packet::Type::REQUEST, packet.req.type == Request::Type::READ);
        // printf("Cub %d Vault %d injected a packet to %d\n",cubIdx, deviceIdx, target_device);
    }
    ...
    void tick()
    {
        ...
        (*rmab_length_sum) += pim_remote_packets_buffer.size();
