...
#include <algorithm>
class CDArray : public CacheArray {
    private:
        //We need a fake replpolicy and just want the CC...
        class ProxyReplPolicy : public ReplPolicy {
            private:
                CDArray* a;
            public:
                explicit ProxyReplPolicy(CDArray* _a) : a(_a) {}
                void setCC(CC* _cc) {a->setCC(cc);}
    ...
    public:
        explicit CDArray(uint32_t _numLines) : numLines(_numLines), cc(nullptr) {
            lineAddrs = gm_calloc<Address>(numLines);
            for (uint32_t i = 0; i < numLines; i++) {
        ...
        uint32_t preinsert(const Address lineAddr, const MemReq* req, Address* wbLineAddr) {
            ...
            uint32_t candidate = freeList.front();
            *wbLineAddr = lineAddrs[candidate];
            return candidate;
        }
        ...
        void postinsert(const Address lineAddr, const MemReq* req, uint32_t lineId) {
            ...
            // assert(lineMap[lineAddr] == lineId);
            // assert(lineMap.find(lineAddr) != lineMap.end());
            // int32_t lineid_t = lookup(lineAddr, req, false);
            // printf("Instered 0x%x to line %d, line_new %d\n",lineAddr, lineId, lineid_t);
        }

        void clear(uint32_t lineId){
            // printf("Clear line %d, 0x%x\n", lineId,array[lineId]);
            // lineMap.erase(lineAddrs[lineId]);
            lineMap[lineAddrs[lineId]] = -1;
            lineAddrs[lineId] = -1;
            assert(std::find(freeList.begin(), freeList.end(), lineId) == freeList.end());
            freeList.push_back(lineId);
        }
