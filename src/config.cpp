...
Config::Config(const char* inFile, libconfig::Config* out_cfg) {
    inCfg = new libconfig::Config();
    outCfg = out_cfg;
    outCfgAllocated = false;
    try {
        inCfg->readFile(inFile);
    } catch (libconfig::FileIOException fioe) {
        panic("Input config file %s could not be read", inFile);
...
void Config::writeAndClose(const char* outFile, Config &conf, bool strictCheck) {
    uint32_t nonSimVars = copyNonSimVars(inCfg->getRoot(), outCfg->getRoot(), std::string(""));
    uint32_t unused = checkIncluded(inCfg->getRoot(), outCfg->getRoot(), std::string(""));

    nonSimVars += copyNonSimVars(conf.getInCfgRoot(), outCfg->getRoot(), std::string(""));
    unused += checkIncluded(conf.getInCfgRoot(), outCfg->getRoot(), std::string(""));
...
libconfig::Setting& Config::getInCfgRoot(){return inCfg->getRoot();}
libconfig::Setting& Config::getOutCfgRoot(){return outCfg->getRoot();}
