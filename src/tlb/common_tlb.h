...
{
	public:
		CommonTlb(const g_string& name, bool enable_timing_mode, unsigned tlb_size , unsigned hit_lat , 
				unsigned res_lat , unsigned line_shift, unsigned page_shift, EVICTSTYLE policy=LRU):tlb_entry_num(tlb_size),
				hit_latency(hit_lat),response_latency(res_lat),tlb_access_time(0), tlb_hit_time(0),tlb_evict_time(0),
				tlb_name_(name),enable_timing_mode(enable_timing_mode), line_shift(line_shift), page_shift(page_shift),page_size(1<<page_shift), evict_policy(policy)
		{
			assert(tlb_size>0);
		...
		uint64_t access( MemReq& req )
		{
			...
			if(!entry)
			{
				...
				insert_num++;
				new_entry.set_valid();
				if(req.pageShared) new_entry.set_page_shared();
				if(req.pageDirty) new_entry.set_page_dirty();
		...
		uint32_t update_tlb_flags(Address ppn, bool shared, bool dirty){
			...
			if(entry)
			{
				if(shared) entry->set_page_shared();
				if(dirty) entry->set_page_dirty();
			}
			uint32_t set_lat=0;
			if(enable_timing_mode)
				set_lat = hit_latency;
		...
		*/
		...
		uint64_t calculate_stats(std::ofstream &vmof)
		{
			...
			vmof<<tlb_name_<< " access time:"<< tlb_access_time <<"\t hit time:"<< tlb_hit_time<<"\t miss time:"<< (tlb_access_time - tlb_hit_time) <<"\t evict time:"<< tlb_evict_time <<"\t hit rate:"<<tlb_hit_rate<<std::endl;;
			return tlb_access_time;
		}
		void clear_counter()
		{
