...
inline void validate_page(PageTable* table , unsigned entry_id , Page* next_level_addr)
{
...
inline void validate_entry(PageTable* table , unsigned entry_id , PageTable* next_level_addr)
{
	//std::cout<<std::hex<<(*table)[entry_id]<<std::endl;
...
/* template<class T>
inline void validate_entry(PageTable* table , unsigned entry_id , T* next_level_addr)
{
	//std::cout<<std::hex<<(*table)[entry_id]<<std::endl;
...
/* inline Address get_block_id(MemReq& req, void* pblock, bool write_back, uint32_t  access_counter)
{
	if(!pblock)
