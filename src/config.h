...
class Config {
    private:
        ...
        bool outCfgAllocated = true;

    public:
        explicit Config(){};
        explicit Config(const char* inFile);
        explicit Config(const char* inFile, libconfig::Config* out_cfg);
        ~Config();
        ...
        void writeAndClose(const char* outFile, Config &conf, bool strictCheck);
        ...
        libconfig::Setting& getInCfgRoot();
        libconfig::Setting& getOutCfgRoot();

        libconfig::Config* getOutCfg(){return outCfg;}
