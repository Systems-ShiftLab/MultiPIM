...
void ContentionSim::postInit() {
    for (uint32_t i = 0; i < zinfo->numCores; i++) {
        PIMCore* pcore = dynamic_cast<PIMCore*>(zinfo->cores[i]);
        if (pcore) {
...
void ContentionSim::enqueue(TimingEvent* ev, uint64_t cycle) {
    ...
    // printf("Enqueuing event @ domain %d cycle %ld curCycle %ld\n", ev->domain,cycle, domains[ev->domain].curCycle);
    assert_msg(cycle >= domains[ev->domain].curCycle, "Queued event goes back in time, domain %d cycle %ld curCycle %ld", ev->domain,cycle, domains[ev->domain].curCycle);
    ev->privCycle = cycle;
...
void ContentionSim::simThreadLoop(uint32_t thid) {
    ...
    while (true) {
        ...
        // info("%d --- phase start", thid);
        simulatePhaseThread(thid);
        // info("%d --- phase end", thid);
