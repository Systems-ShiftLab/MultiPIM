...
SimpleCore::SimpleCore(FilterCache* _l1i, FilterCache* _l1d, g_string& _name) : 
                Core(_name), l1i(_l1i), l1d(_l1d), instrs(0), curCycle(0), haltedCycles(0) {
}
...
void SimpleCore::LoadFunc(THREADID tid, ADDRINT addr, UINT32 size, BOOL inOffloadRegion) {
    if(zinfo->enable_pim_mode && zinfo->skipNonOffloadBBL && !inOffloadRegion)
        return;
    static_cast<SimpleCore*>(cores[tid])->load(addr);
}

void SimpleCore::StoreFunc(THREADID tid, ADDRINT addr, UINT32 size, BOOL inOffloadRegion) {
    if(zinfo->enable_pim_mode && zinfo->skipNonOffloadBBL && !inOffloadRegion)
        return;
    static_cast<SimpleCore*>(cores[tid])->store(addr);
}

void SimpleCore::PredLoadFunc(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, BOOL inOffloadRegion) {
    if(zinfo->enable_pim_mode && zinfo->skipNonOffloadBBL && !inOffloadRegion)
        return;
    if (pred) static_cast<SimpleCore*>(cores[tid])->load(addr);
}

void SimpleCore::PredStoreFunc(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, BOOL inOffloadRegion) {
    if(zinfo->enable_pim_mode && zinfo->skipNonOffloadBBL && !inOffloadRegion)
        return;
    if (pred) static_cast<SimpleCore*>(cores[tid])->store(addr);
}

void SimpleCore::BblFunc(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo, BOOL inOffloadRegion) {
    if(zinfo->enable_pim_mode && zinfo->skipNonOffloadBBL && !inOffloadRegion)
        return;
