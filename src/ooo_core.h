...
class OOOCore : public Core {
    private:
    	...
    	uint64_t offload_instrs, offlod_mem_instrs;
        ...
        uint64_t instrs, mem_instrs, uops, bbls, approxInstrs, mispredBranches;
        ...
        bool offloadRS[MAX_REGISTERS];
        bool offloadRD[MAX_REGISTERS];

        MemObject* offloadMC;
        BBLStatus* threadBBLStatus;
    public:
        OOOCore(FilterCache* _l1i, FilterCache* _l1d, g_string& _name);
        OOOCore(FilterCache* _l1i, FilterCache* _l1d, g_string& _name, bool enable_pim_mode);

        void offloadFunction_begin();
        void offloadFunction_end();
        inline int get_offload_code();
        void pre_offload();
        uint64_t do_offload(uint64_t curCycle);
        ...
        uint64_t getBoundCycles() const {return MAX(curCycle, zinfo->globPhaseCycles);}
	    uint64_t getOffloadInstrs() const; //LOIS
        uint64_t getMemInstrs() const;
        ...
        BaseTlb* getInsTlb();
		BaseTlb* getDataTlb();
        BaseCache* getInsCache() {return (BaseCache*)(l1i);}
		BaseCache* getDataCache() {return (BaseCache*)(l1d);}
        BaseCoreRecorder* getCoreRecorder() {return (BaseCoreRecorder*)&cRec;}
        uint64_t clflush( Address startAddr, uint64_t startCycle);
    private:
        ...
        static void LoadFunc(THREADID tid, ADDRINT addr, UINT32 size, BOOL inOffloadRegion);
        static void StoreFunc(THREADID tid, ADDRINT addr, UINT32 size, BOOL inOffloadRegion);
        static void PredLoadFunc(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, BOOL inOffloadRegion);
        static void PredStoreFunc(THREADID tid, ADDRINT addr, BOOL pred, UINT32 size, BOOL inOffloadRegion);
        static void BblFunc(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo, BOOL inOffloadRegion);
        static void BranchFunc(THREADID tid, ADDRINT pc, BOOL taken, ADDRINT takenNpc, ADDRINT notTakenNpc, BOOL inOffloadRegion);
