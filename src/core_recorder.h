...
class BaseCoreRecorder{
    public:
        virtual void record(uint64_t startCycle, uint64_t dispatchCycle, uint64_t respCycle) = 0;
};

class CoreRecorder : public BaseCoreRecorder {
    private:
        typedef enum {
    ...
    public:
        ...
        inline void record(uint64_t startCycle, uint64_t dispatchCycle, uint64_t respCycle){
            if (unlikely(eventRecorder.hasRecord())) recordAccess(startCycle);
        }
