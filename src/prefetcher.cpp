        ...
        if (e.stride == stride) {
            ...
            if (e.conf.pred()) {  // do prefetches
                ...
                if (prefetchPos < 64 && !e.valid[prefetchPos]) {
                    MESIState state = I;
                    MemReq pfReq = {req.lineAddr + prefetchPos - pos, GETS, req.childId, &state, reqCycle, req.childLock, state, req.srcId, MemReq::PREFETCH, req.coreId};
                    pfReq.threadId = req.threadId;
                    pfReq.isPIMInst = req.isPIMInst;
    ...
    for(i=1; i<=16; i++){
        ...
        if((ampm_pages[page_index].access_map[check_index1]==1) && (ampm_pages[page_index].access_map[check_index2]==1)){
            ...
            unsigned long long int pf_address = (page<<6)+(pf_index);
                ...
                MESIState state = I;
                MemReq pfReq = {pf_address, GETS, req.childId, &state, reqCycle, req.childLock, state, req.srcId, MemReq::PREFETCH, req.coreId};
                pfReq.threadId = req.threadId;
                pfReq.isPIMInst = req.isPIMInst;
                uint64_t pfRespCycle = parents[parentId]->access(pfReq);  // FIXME, might segfault
