...
uint64_t TimingCache::access(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        TimingRecord tr = {req.lineAddr << lineBits, req.cycle, respCycle, req.type, nullptr, nullptr,req.threadId, req.is(MemReq::Flag::PTW)}; //note the end event is the response, not the wback
