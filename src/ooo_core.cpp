...
OOOCore::OOOCore(FilterCache* _l1i, FilterCache* _l1d, g_string& _name, bool _enable_pim_mode) : 
    ...
    if(enable_pim_mode){
        int cores_per_group = zinfo->numCores / zinfo->numMemoryControllers;
        int group_idx = coreIdx / cores_per_group;
        offloadMC = zinfo->memoryControllers[group_idx];
        assert(offloadMC);
    }else{
        offloadMC = nullptr;
    }
...
void OOOCore::initStats(AggregateStat* parentStat) {
    ...
    memInstrsStat->init("memInstrs", "Simulated memory instructions", &mem_instrs);
    ...
    coreStat->append(offloadInstrsStat);
    coreStat->append(offloadMemInstrsStat);
...
uint64_t OOOCore::getMemInstrs() const {return mem_instrs;};
uint64_t OOOCore::getPhaseCycles() const {return curCycle % zinfo->phaseLength;}

BaseTlb* OOOCore::getInsTlb() {return l1i->getTlb();}

BaseTlb* OOOCore::getDataTlb() {return l1d->getTlb();}
...
void OOOCore::contextSwitch(int32_t gid) {
    if (gid == -1) {//deschedule
        ...
        //should also save/store scoreboard
        zinfo->sched->saveRegScoreBoard(procIdx, threadIdx, regScoreboard);
    }else{
        //new task(thread)
        zinfo->sched->restoreRegScoreBoard(gid, regScoreboard);
    }
}

void OOOCore::flushTlb(){
    l1i->flushTlb();
    l1d->flushTlb();
}
...
void OOOCore::offloadFunction_begin()
{
...
void OOOCore::offloadFunction_end()
{
...
int OOOCore::get_offload_code()
{
    // assert(threadBBLStatus);
    return threadBBLStatus->offload_code;
}

void OOOCore::pre_offload()
{
    assert(enable_pim_mode);
    OffLoadMetaData *metadata;
    threadBBLStatus->pre_offload = false;
    if(zinfo->tlb_enabled)
        metadata = new OffLoadMetaData(procIdx, threadIdx,zinfo->paging_array[procIdx], NULL);
    else
        metadata = new OffLoadMetaData(procIdx, threadIdx, NULL, zinfo->memoryMapper);
    threadBBLStatus->pim_task = metadata;
    for (uint32_t i = 0; i < MAX_REGISTERS; i++) {
        offloadRS[i] = false;
        offloadRD[i] = false;
    }
...
uint64_t OOOCore::do_offload(uint64_t curCycle)
{
    assert(enable_pim_mode);
    assert(threadBBLStatus->offload_code == 0);
    threadBBLStatus->pim_task->cycle = curCycle;
    threadBBLStatus->pim_task->srcId = coreIdx;
    uint64_t reqSatisfiedCycle = offloadMC->offload(threadBBLStatus->pim_task);
    threadBBLStatus->pim_task = NULL;
    threadBBLStatus->pre_offload = true;
    threadBBLStatus->need_offload = false;
    threadBBLStatus->can_offload = false;
    // printf("Do_offload\n");
    // printf("Process %d Thread %d offloaded a task\n",procIdx, threadIdx);
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo, bool cur_pim_bbl) {
    ...
    if(zinfo->maxOffloadInstrsIssued){
        ...
        advance(curCycle + zinfo->phaseLength);
        loads = stores = 0;
        return;
    }
    ...
    Address bblStartAddr = prevAddr;
    uint32_t bblBytes = prevBbl->bytes;
    BblInfo* curBbl = prevBbl;
    prevBbl = bblInfo;
    prevAddr = bblAddr;
            ...
            case UOP_LOAD:
                {
                    ...
                    if (addr != ((Address)-1L)) {
                        ...
                        reqSatisfiedCycle = l1d->load(addr, dispatchCycle, procIdx, threadIdx, false) + L1D_LAT;
                        cRec.record(curCycle, dispatchCycle, reqSatisfiedCycle);
                        // std::cout<<"(Non-PIM BBL1) curCycle "<< curCycle<<" dispatchCycle "<<dispatchCycle<< " reqSatisfiedCycle " << reqSatisfiedCycle << std::endl;
                    }
            ...
            case UOP_STORE:
                {
                    ...
                    storeIdx++;
                    uint64_t reqSatisfiedCycle = l1d->store(addr, dispatchCycle, procIdx, threadIdx, false) + L1D_LAT;
                    cRec.record(curCycle, dispatchCycle, reqSatisfiedCycle);
                    // std::cout<<"(Non-PIM BBL2) curCycle "<< curCycle<<" dispatchCycle "<<dispatchCycle<< " reqSatisfiedCycle " << reqSatisfiedCycle << std::endl;
    ...
    if(cur_pim_bbl){
        ...
        offlod_mem_instrs += loads;
        offlod_mem_instrs += stores;
    }
    mem_instrs += loads;
    mem_instrs += stores;
    instrs += bblInstrs;
    ...
    if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
        ...
        for (uint32_t i = 0; i < 5*64/FETCH_WIDTH; i++) {
            uint64_t fetchLat = l1i->load(wrongPathAddr + FETCH_WIDTH*i, curCycle, procIdx, threadIdx,false) - curCycle;
            ...
            reqCycle = respCycle + FETCH_WIDTH/FETCH_BYTES_PER_CYCLE;
        }
    ...
    Address endAddr = bblAddr + bblInfo->bytes + FETCH_WIDTH -1;
    for (Address fetchAddr = bblAddr; fetchAddr < endAddr; fetchAddr += FETCH_WIDTH) {
        ...
        uint64_t fetchLat = l1i->load(fetchAddr, curCycle, procIdx, threadIdx,false) - curCycle;
        cRec.record(curCycle, curCycle, curCycle + fetchLat);
        fetchCycle += fetchLat;
    }
...
{
    Address cacheline = startAddr >> lineBits;

    uint64_t respCycle =  l1d->clflush( cacheline, startCycle, true);
