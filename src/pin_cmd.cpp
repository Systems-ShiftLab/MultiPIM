...
PinCmd::PinCmd(Config* conf, const char* configFile, const char* outputDir, uint64_t shmid) {
    ...
    const char* libZsimEnvPath = getenv("LIB_ZSIM_PATH");
    g_string pinPath, zsimPath;
    if (zsimEnvPath && libZsimEnvPath) {
        info("Using env path %s", zsimEnvPath);
        pinPath = zsimEnvPath;
        pinPath += "/pin/intel64/bin/pinbin";
        zsimPath = libZsimEnvPath;
        zsimPath += "/libzsim.so";
    } else {
    ...
    args.push_back("-harnessPid");
    args.push_back(std::to_string(getpid()).c_str());
...
PinCmd::PinCmd(Config* conf, const char* configFile, Config* proc_conf, const char* procFile, const char* outputDir, uint64_t shmid, const char* profileFile) {
        //Figure the program paths
    const char* zsimEnvPath = getenv("ZSIM_PATH");
    const char* libZsimEnvPath = getenv("LIB_ZSIM_PATH");
    g_string pinPath, zsimPath;
    ...
    if (procFile) {
        //Check configFile is an absolute path
        //NOTE: We check rather than canonicalizing it ourselves because by the time we're created, we might be in another directory
        char* absPath = realpath(procFile, nullptr);
        if (std::string(procFile) != std::string(absPath)) {
            panic("Internal zsim bug, procFile should be absolute");
        }
        free(absPath);

        args.push_back("-procConfig");
        args.push_back(procFile);
    }
    ...
    while (true) {
        ...
        if (!proc_conf->exists(p_ss.str().c_str())) break;

        const char* cmd = proc_conf->get<const char*>(p_ss.str() +  ".command");
        const char* input = proc_conf->get<const char*>(p_ss.str() +  ".input", "");
        const char* loader = proc_conf->get<const char*>(p_ss.str() +  ".loader", "");
        const char* env = proc_conf->get<const char*>(p_ss.str() +  ".env", "");
