...
class HistogramCounter : public VectorCounter {
    private:
        uint64_t minValue;
        uint64_t maxValue;
        uint64_t bucketSize;
    ...
    public:
        HistogramCounter() : VectorCounter() {}

        /* Without counter names */
        void init(const char* name, const char* desc, uint32_t size, uint64_t _minValue, uint64_t _maxValue) {
            VectorCounter::init( name, desc, size+1);
            minValue = _minValue;
            maxValue = _maxValue;
            bucketSize = (maxValue - minValue)/size;
        }

        inline void sample(uint64_t value) {
            int id = (value - minValue)/bucketSize;
            if(id >= size())
                id = size() - 1;
            inc(id);
        }
